...
if (all(P_noNA %in% c(0, 1))) {
	    Fam = "binomial"
    Counts = table(P)
        if (length(Counts) < 2) { print("Too few samples") ;  return() }
        if (Counts[2] < 10 | Counts[1] < 10 ){ print("Too few samples") ; return() }
} else { Fam = "gaussian" }
...
print("Fitting model per continent")
#Metadata %>% filter(Continent == "Africa") -> Metadata
for (continent in unique(Metadata$Continent)){
      C = Covariates[!Covariates == "Continent"]
      D2 = paste0(D,"/" ,paste(C,collapse=",") ,"_",continent, "/" )
      print(D2)
      M = Metadata %>% filter(Continent == continent)


      Tr = keep.tip(pruned_tree, M$ID_anal)
      C = Covariates[!Covariates == "Continent"]
      Fit_model(Meta=M, Phenotype_name=Disease, Tree=Tr, COV=C, Dir=D2)
      Dir_flag = paste0("Results/", D2, "Flags")
      print(D2);print(Dir_flag)
      if (!file.exists( Dir_flag  )) { dir.create(Dir_flag) }
      file.create( paste0(Dir_flag, "/", SGB,"_", Disease ) )
