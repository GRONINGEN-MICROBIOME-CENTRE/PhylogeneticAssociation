...
p + geom_tippoint(aes(color=infant), size=0.5)  + scale_color_manual(values = c("#FF7F7F","grey", "black" )) +   new_scale_fill() + geom_fruit( geom="geom_tile", mapping = aes(fill=phylo_effect_median), width=0.1,offset=) + scale_fill_viridis_c(option = "magma") 
...
Phenotypes %>% filter(infant==T) %>% filter(!is.na(sample_id)) %>% mutate(suis_clade = ifelse(sample_id %in% Tree_sub3$tip.label[Check] , T, ifelse(sample_id %in% Tree_sub2$tip.label, F, NA) )  ) %>% filter(! (is.na(suis_clade) ) ) %>%
...
```
What is the average ANI between every two subspecies
```{r }
