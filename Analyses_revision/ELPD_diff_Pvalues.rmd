...
sim_df |>
  ...
  geom_segment(aes(x = elpd_diff - 2*elpd_se,
                   ...
                   yend = i)) +
  theme_light() -> P1
sim_df %>% ggplot(aes(x=elpd_diff)) + geom_histogram() +theme_bw() -> P2
...
```{r }
sim_df %>% mutate(T_stat = elpd_diff/elpd_se, P_value = T_stat_P(T_stat), P_bonferroni = P_value*dim(sim_df)[1], FDR=p.adjust(P_value, "fdr")  )
