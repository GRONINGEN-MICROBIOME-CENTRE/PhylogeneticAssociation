...
Iterate_phenotypes = function(Threshold_no_NA, Threshold_n_cases, Fit_anpan){
	...
	for ( Phenotype in For_association ){
  		...
  		if (Fit_anpan == T){
  			Function_fit_anpan(Phenotype, pruned_tree, Metadata, Fam=Family, Model_save = Save_model, Plot= paste0(Plot_dir, "/", Phenotype, ".pdf" )  ) %>% rbind(Results, . ) -> Results
  		} else {
...
Run_single_pheno = function(Phenotype, Fit_anpan){
	...
	print(paste0(Phenotype, " for association") )
        if (all(P_noNA %in% c(0, 1))) { Family = "binomial"
...
Run_single_pheno(Phenotype, Fit_anpan)
